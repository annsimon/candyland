...
namespace Candyland
{
    ...
    {
        public static Dictionary<string, Area> ParseAreas(UpdateInfo info, Camera cam)
        {
            ...
            foreach( XmlNode node in id )
            {
                ...
                Area area = new Area(node.InnerText, startPos, info, cam, areaContent[count].InnerXml);
